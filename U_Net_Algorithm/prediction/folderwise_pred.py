...
class LesionPredictionPipeline:
  ...
  def __init__(self, weights_path, image_size=(512, 512), threshold=0.5):
    ...
    # Initialize sub-layers via dummy forward pass (Keras 3 safe)
  ...
  def run_prediction(self, image_dir, output_dir):
    ...
    for root, _, files in os.walk(image_dir):
      ...
      overlay_dir = os.path.join(current_output_dir, "overlay")
      os.makedirs(overlay_dir, exist_ok=True)

      for cls in self.classes:
        os.makedirs(
            os.path.join(current_output_dir, "masks", cls), exist_ok=True
        )
      ...
      for img_name in image_files:
        ...
        overlay_canvas = cv2.resize(original_img, (orig_w, orig_h))

        # Generate class binary masks & visual overlay
        for i, cls in enumerate(self.classes):
          ...
          mask_out_path = os.path.join(
              current_output_dir, "masks", cls, img_name
          )
          cv2.imwrite(mask_out_path, resized_mask)

          # Apply color overlay
          mask_indices = resized_mask > 0
          if np.any(mask_indices):
            color = self.class_colors[cls]
            overlay_canvas[mask_indices] = cv2.addWeighted(
                overlay_canvas[mask_indices],
                0.4,
                np.full_like(overlay_canvas[mask_indices], color),
                0.6,
                0,
            )

        # Save visual overlay
        cv2.imwrite(os.path.join(overlay_dir, img_name), overlay_canvas)

    print(
        f"\n[✓] Lesion Segmentation predictions completed! Check output:"
        f" {output_dir}"
    )
...
if __name__ == "__main__":
  input_weights_path = "dropfilter_unet_best.weights.h5"
  image_dir = r"..\Output\data_balancing"
  output_dir = r"..\Output\lession_segmentation"
